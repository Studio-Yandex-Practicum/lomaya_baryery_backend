...
3. Запустить `python run.py` — приложение само зарегистрирует подписку
   `POST /subscriptions` на адрес `{APPLICATION_URL}/api/max/webhook/{MAX_WEBHOOK_SECRET}`
   и снимет её при остановке.

> **Note**:
> У API Max нет аналога секретного заголовка telegram, поэтому секрет передаётся
> в пути вебхука. Значение берётся из `MAX_WEBHOOK_SECRET`; если переменная не задана,
> подставляется случайное значение при старте. `SECRET_KEY` для этого не используется:
> путь вебхука попадает в логи веб-сервера и в реестр подписок Max, а `SECRET_KEY`
