...
class ShiftRepository(AbstractRepository):
    ...
    async def get_with_members(self, id: UUID, member_status: Optional[Member.Status]) -> Shift:
        ...
        if request is None:
            raise exceptions.NotFoundError(object_name=Shift.__doc__, object_id=id)
        return request
    ...
    async def get_started_shift_id(self) -> UUID:
        ...
        if not shift_id:
            raise exceptions.GetStartedShiftError(detail='Активной смены не найдено.')
        return shift_id
    ...
    async def get_open_for_registration_shift_id(self) -> UUID:
        ...
        if not shift_id:
            raise exceptions.RegistrationForbidenError
        return shift_id
