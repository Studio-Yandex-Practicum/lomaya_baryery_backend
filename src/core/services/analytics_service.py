...
class AnalyticsService:
    ...
    def __init__(
        self, task_repository: TaskRepository = Depends(), task_report_builder: AnalyticReportBuilder = Depends()
    ) -> None:
    ...
    async def __generate_task_report(self, workbook: Workbook) -> Workbook:
        ...
        await self.__task_report_builder.generate_report(
            tasks_statistic, workbook=workbook, analytic_task_report_full=TaskAnalyticReportSettings
        )
